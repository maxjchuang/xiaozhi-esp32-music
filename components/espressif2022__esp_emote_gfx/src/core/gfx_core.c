...
static inline int gfx_min_int(int a, int b) { return a < b ? a : b; }
static inline int gfx_max_int(int a, int b) { return a > b ? a : b; }

// Forward declarations
static bool gfx_refr_handler(gfx_core_context_t *ctx);
static bool gfx_event_handler(gfx_core_context_t *ctx);
static bool gfx_object_handler(gfx_core_context_t *ctx, gfx_area_t *dirty_area);
...
static bool gfx_object_handler(gfx_core_context_t *ctx, gfx_area_t *dirty_area)
{
    if (ctx->disp.child_list == NULL || dirty_area == NULL) {
        return false;
    }

    bool updated = false;
    dirty_area->x1 = ctx->display.h_res;
    dirty_area->y1 = ctx->display.v_res;
    dirty_area->x2 = 0;
    dirty_area->y2 = 0;
    ...
    while (child_node != NULL) {
        ...
        // Hidden objects are not drawn, so their dirty flag cannot be cleared
        // by a draw handler. Ignoring them here also prevents invisible
        // animations from continuously scheduling full-screen refreshes.
        if (!obj->is_visible) {
        ...
        if (obj->is_dirty) {
            // Animation frames may change their decoded dimensions and can
            // leave pixels from the previous frame behind.  Keep animations
            // on the component's original full-screen refresh path; dirty
            // rectangles are reserved for stable image/label objects such as
            // the music artwork scene.
            if (obj->type == GFX_OBJ_TYPE_ANIMATION) {
                dirty_area->x1 = 0;
                dirty_area->y1 = 0;
                dirty_area->x2 = ctx->display.h_res;
                dirty_area->y2 = ctx->display.v_res;
                updated = true;
                child_node = child_node->next;
                continue;
            }
            ...
            gfx_obj_calculate_aligned_position(obj, ctx->display.h_res,
                                               ctx->display.v_res, &obj_x, &obj_y);
            if (obj->width == 0 || obj->height == 0) {
                ...
                dirty_area->x1 = gfx_min_int(dirty_area->x1, gfx_max_int(0, obj_x));
                dirty_area->y1 = gfx_min_int(dirty_area->y1, gfx_max_int(0, obj_y));
                dirty_area->x2 = gfx_max_int(
                    dirty_area->x2,
                    gfx_min_int((int)ctx->display.h_res, obj_x + obj->width));
                dirty_area->y2 = gfx_max_int(
                    dirty_area->y2,
                    gfx_min_int((int)ctx->display.v_res, obj_y + obj->height));
            }
            updated = true;
        }
        child_node = child_node->next;
    ...
    return updated && dirty_area->x1 < dirty_area->x2 && dirty_area->y1 < dirty_area->y2;
}
 ...
 */
static int gfx_buf_get_height(gfx_core_context_t *ctx, int render_width)
{
    if (ctx->disp.buf_pixels == 0 || render_width <= 0) {
        return 0;
    }
    return ctx->disp.buf_pixels / render_width;
}
...
static bool gfx_refr_handler(gfx_core_context_t *ctx)
{
    gfx_area_t dirty_area;
    bool updated = gfx_object_handler(ctx, &dirty_area);

    if (!updated) {
        return false;
    }

    const int render_width = dirty_area.x2 - dirty_area.x1;
    int block_height = gfx_buf_get_height(ctx, render_width);
    if (block_height == 0) {
        ESP_LOGE(TAG, "Invalid frame buffer size");
        return false;
    }

    const int render_height = dirty_area.y2 - dirty_area.y1;
    int total_blocks = (render_height + block_height - 1) / block_height;

    for (int block_idx = 0; block_idx < total_blocks; block_idx++) {
        int x1 = dirty_area.x1;
        int x2 = dirty_area.x2;

        int y1 = dirty_area.y1 + block_idx * block_height;
        int y2 = gfx_min_int(dirty_area.y2, y1 + block_height);
        ...
        size_t pixels = (size_t)(x2 - x1) * (y2 - y1);
