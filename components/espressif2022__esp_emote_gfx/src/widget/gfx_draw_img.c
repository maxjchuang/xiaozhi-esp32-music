...
void gfx_draw_img(gfx_obj_t *obj, int x1, int y1, int x2, int y2, const void *dest_buf, bool swap)
{
    ...
    const gfx_coord_t source_offset_x = clip_region.x1 - obj_x;
    const gfx_coord_t source_offset_y = clip_region.y1 - obj_y;
    gfx_color_t *source_pixels = (gfx_color_t *)(
        image_data + (source_offset_y * source_buffer_stride + source_offset_x) * 2);
    gfx_opa_t *alpha_mask = (gfx_opa_t *)(
        image_data + source_buffer_stride * image_height * 2 +
        source_offset_y * source_buffer_stride + source_offset_x);
